...
bool Game::Initialize (GL_Window* window, Keys* keys, DWORD tickCount)	
{
	...
	textObject.SetText ("Adok's Saturn Puzzle");
	...
	textObject.SetText ("Use cursor keys to rotate the sphere. Press Space to rotate numbers. Press Esc to enter a menu where you can save and load game, start new game, etc. The game is won once all the numbers are in order.");
	...
	for (int i = 0; i < 13; i++)
	{
		if (!bitmap_number [i].New (64, 64))
			return false;
		bitmap_number [i].SetRenderTextFont ("Verdana", 64);
		bitmap_number [i].SetRenderTextColor (0x000001);
		if (i + 1 >= 10)
		{
			number [0] = '0' + (i + 1) / 10;
			number [1] = '0' + (i + 1) % 10;
		...
		textObject.SetText (number);
		bitmap_number [i].RenderText (&textObject, Align_Center);
...
void Game::NewGame ()
{
	...
	sphere.NewGame (texture_number, selectedDifficulty);
...
void Game::EnterMenu (DWORD tickCount)
{
...
inline void Game::Update_ActualGame (DWORD tickCount, DWORD lastTickCount)
{
	...
	if (targetAngle == 0 && !frontFaceRotating)
	{
			...
			g_keys->keyDown [VK_RETURN])
		{
			...
			else if (g_keys->keyDown [VK_SPACE] && !victory)
				frontFaceRotating = true;
			else if (cheatMode && g_keys->keyDown [VK_RETURN] && !victory)
			{
				if (sphere.UndoMove ())
				{
					frontFaceRotating = true;
	...
	if (frontFaceRotating)
	{
		...
		if (angleFrontFace >= 180)
		{
			frontFaceRotating = false;
			angleFrontFace = 0;
			sphere.RotateFields (updateHistory);
			steps++;
			UpdateStepsDisplay ();
			if (sphere.IsSolved ())
	...
	{
		angle = targetAngle * (tickCount - rotationStartTickCount) / rotationDelay;
		if (abs (angle) >= abs (targetAngle) * 360 / 13)
		{
			targetAngle = 0;
			angle = 0;
			sphere.Rotate (dir);
...
inline void Game::Update_Load (DWORD tickCount, DWORD lastTickCount)
{
	if (g_keys->keyDown [VK_ESCAPE])
		escPressed = true;
...
inline void Game::Update_Save (DWORD tickCount, DWORD lastTickCount)
{
	if (g_keys->keyDown [VK_ESCAPE])
		escPressed = true;
...
inline void Game::Update_Difficulty (DWORD tickCount, DWORD lastTickCount)
{
	if (g_keys->keyDown [VK_ESCAPE])
		escPressed = true;
...
inline void Game::Update_Menu (DWORD tickCount, DWORD lastTickCount)
{
...
inline void Game::Update_Instructions (DWORD tickCount, DWORD lastTickCount)
{
...
inline void Game::Update_Credits (DWORD tickCount, DWORD lastTickCount)
{
	Update_Instructions (tickCount, lastTickCount);
}
...
inline void Game::Draw_ActualGame (DWORD tickCount)
{
	...
	sphere.Draw (angleFrontFace, angle);
}

inline void Game::Draw_Load (DWORD tickCount)
{
	glClear (GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
inline void Game::Draw_Save (DWORD tickCount)
{
	glClear (GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
inline void Game::Draw_Difficulty (DWORD tickCount)
{
	glClear (GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
inline void Game::Draw_Menu (DWORD tickCount)
{
	glClear (GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
inline void Game::Draw_Instructions (DWORD tickCount)
{
	glClear (GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
inline void Game::Draw_Credits (DWORD tickCount)
{
	glClear (GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
inline void Game::EnterVictory (DWORD tickCount)
{
...
bool Game::SaveGame (DWORD tickCount)
{
	FILE *file;
	char filename [13];
...
void Game::LoadGame (int number, DWORD tickCount)
{
	char filename [13];
	...
	sphere.Load (file, texture_number);
...
void Game::LeaveActualGame (DWORD tickCount)
{
	if (steps)
		elapsedTime += tickCount - startTime;
}

void Game::ReturnToActualGame (DWORD tickCount)
{
	startTime = tickCount;
	if (!music_playing)
	{
